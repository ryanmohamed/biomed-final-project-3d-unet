...
USES 17 IMAGES ADDED TO THE TRAINING DATA DIRECTORY
PROCESSES AND ADDS TO A NEW PARTITION
12 IMAGES FOR TRAINING
5 IMAGES FOR TESTING
SPLIT IS DONE MANUALLY

USED FOR VALIDATION PROCESSING AS WELL
HARDCODED PATHS ARE REPLACED 
...
output_path = '/Users/ryan/Desktop/BIOMEDICAL IA/FINAL PROJECT/code/combined/'
...
# process each one
for img_num in range(len(segmentation_paths)):
    
    print('Processing image ', img_num)

    # load images for current 
    t1ce = nb.loadsave.load(t1ce_paths[img_num]).get_fdata()
    t2 = nb.loadsave.load(t2_paths[img_num]).get_fdata()
    flair = nb.loadsave.load(flair_paths[img_num]).get_fdata()
    segmentation = nb.loadsave.load(segmentation_paths[img_num]).get_fdata()
